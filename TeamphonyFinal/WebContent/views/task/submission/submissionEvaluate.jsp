...
.point>.input,
.point>.input>label:hover,
.point>.input>input:focus+label,
.point>.input>input:checked+label{
...
.point>.input>label:hover~label{
    background-image: none;
}
.point>.input>label[for="p1"]{ 
    width: 10px;
    z-index: 10;
}
.point>.input>label[for="p2"]{ 
    width: 20px;
    z-index: 9;
}
.point>.input>label[for="p3"]{ 
    width: 30px;
    z-index: 8;
}
.point>.input>label[for="p4"]{ 
    width: 40px;
    z-index: 7;
}
.point>.input>label[for="p5"]{ 
    width: 50px;
    z-index: 6;
}
.point>.input>label[for="p6"]{ 
    width: 60px;
    z-index: 5;
}
.point>.input>label[for="p7"]{ 
    width: 70px;
    z-index: 4;
}
.point>.input>label[for="p8"]{ 
    width: 80px;
    z-index: 3;
}
.point>.input>label[for="p9"]{ 
    width: 90px;
    z-index: 2;
}
.point>.input>label[for="p10"]{ 
    width: 100px;
    z-index: 1;
}
.point>output{
...
<a href="${pageContext.request.contextPath}/submission/searchAll.do">
	<button type="reset"   class="w3-btn w3-white w3-border w3-border-blue w3-text-blue w3-round-large">제출과제 리스트</button>
</a>
	</div>
</div>
<form action="${pageContext.request.contextPath}/submission/evaluate.do" method="post">
				...
				<td>	
					<span class="point">
						<span class="input">
							...
							<input type="hidden" name="evaluated" id="evaluated" value="1" >
	...
	<div align="right">
		<button type="submit" class="w3-btn w3-white w3-border w3-border-orange w3-text-orange w3-round-large">평가완료</button>
	</div>
